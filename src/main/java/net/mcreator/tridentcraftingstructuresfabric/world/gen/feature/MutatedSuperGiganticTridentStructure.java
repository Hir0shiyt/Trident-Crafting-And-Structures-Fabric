...
public class MutatedSuperGiganticTridentStructure {
		...
		public boolean generate(StructureWorldAccess worldAccess, ChunkGenerator chunkGenerator, Random random, BlockPos pos,
			...
			if ((random.nextInt(1000000) + 1) <= 450) {
				...
				for (int a = 0; a < count; a++) {
					...
					int j = worldAccess.getTopY(Heightmap.Type.WORLD_SURFACE_WG, i, k);
					...
					Structure structure = world.getStructureManager()
							.getStructureOrBlank(new Identifier("tridentcraftingstructuresfabric", "mutated_giant_falling_trident"));
					if (structure == null)
						return false;
					...
					structure.place(worldAccess, spawnTo,
							new StructurePlacementData().setRotation(rotation).setMirror(mirror).setRandom(random)
									.addProcessor(BlockIgnoreStructureProcessor.IGNORE_AIR_AND_STRUCTURE_BLOCKS).setChunkPosition(null)
	...
	public static void init() {
		Registry.register(Registry.FEATURE, new Identifier("tridentcraftingstructuresfabric", "mutated_super_gigantic_trident"), feature);
		RegistryKey<ConfiguredFeature<?, ?>> configFeatKey = RegistryKey.of(Registry.CONFIGURED_FEATURE_WORLDGEN,
				new Identifier("tridentcraftingstructuresfabric", "mutated_super_gigantic_trident"));
		Registry.register(BuiltinRegistries.CONFIGURED_FEATURE, configFeatKey.getValue(), configFeature);
		BiomeModifications.addFeature(
				BiomeSelectors.includeByKey(BiomeKeys.COLD_OCEAN, BiomeKeys.DEEP_COLD_OCEAN, BiomeKeys.DEEP_FROZEN_OCEAN,
						BiomeKeys.DEEP_LUKEWARM_OCEAN, BiomeKeys.DEEP_OCEAN, BiomeKeys.DEEP_WARM_OCEAN, BiomeKeys.OCEAN),
				GenerationStep.Feature.RAW_GENERATION, configFeatKey);
	}
}
