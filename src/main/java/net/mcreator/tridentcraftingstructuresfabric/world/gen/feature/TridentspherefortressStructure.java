...
public class TridentspherefortressStructure {
			...
			if ((random.nextInt(1000000) + 1) <= 900) {
				...
				for (int a = 0; a < count; a++) {
					...
					Structure structure = world.getStructureManager()
							.getStructureOrBlank(new Identifier("tridentcraftingstructuresfabric", "1k_special_structure"));
					if (structure == null)
						return false;
	...
	public static void init() {
		Registry.register(Registry.FEATURE, new Identifier("tridentcraftingstructuresfabric", "tridentspherefortress"), feature);
		RegistryKey<ConfiguredFeature<?, ?>> configFeatKey = RegistryKey.of(Registry.CONFIGURED_FEATURE_WORLDGEN,
				new Identifier("tridentcraftingstructuresfabric", "tridentspherefortress"));
		Registry.register(BuiltinRegistries.CONFIGURED_FEATURE, configFeatKey.getValue(), configFeature);
		BiomeModifications.addFeature(BiomeSelectors.all(), GenerationStep.Feature.RAW_GENERATION, configFeatKey);
	}
}
